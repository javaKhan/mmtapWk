...
OrderInfoDlg.collectData = function() {
    this.set('customname').set('wwid').set('address').set('mobile')
        .set('netid').set('place').set('cuscom');
    var bus_array = [];
    $('input[name="buss"]:checked').each(function (i) {
        bus_array[i]=$(this).val();
    });
    this.set('buss',bus_array).set('ocomment');
}
